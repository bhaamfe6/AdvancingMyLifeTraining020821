# Java Data Types
At a high level, there are two main groups of data types. There are the following:
...
Think of chemistry, for example.  In our various mixtures we can have a mix of compounds, molecules, atoms, etc., but ultimately at some point, everything reduces down to atoms. Atoms are analougous to the idea primitives.
...
Passes by value and sgtored by value is important to understand for primitives, buecause not all languages treat primitive data types this way.
...
| int | 32 bits | -2.147,483,648 (or -2^31) | 2,147,483,647 ( or 2^31-1) | 0 |
...
- Reference types do not actually contain the object itself, but instead contain a "refernce" to the location of the object (memory address)
    - Think of reference types as a variable holding information on where an object is located.
...
- Explicit casting
- Implicit casting
Casting can be performed on both primitive and reference data types.  Although primitive conversion and reference variable casting look similar, they are actually very differenct conceptually.
...
Primitive casting is also known as type conversion.  This allows us to change the value from one data type to another.  
- Implicit casting occurs if we go from a primitive type that is "smaller" in range to one that is "larger" in range.
    -  In more technical terms, implicit casting occurs when the casting operation is "safe"
    - Explicit casting occurs when a higher primitive data type is converted to a lower primitive data type and this is necessary if it is "unsafe".
- Widening conversion
    - Occurs implicitly
    - Care should be taken when converting from a wider primitive to a narrower primitive 
    - Example:  
         double-> float-> long-> int-> char-> short-> byte-> bit
...
- Unlike primitive casting, reference casting doesn't modifty or touch the object itself, only the variable that is pointing to the object
    - Reference casting simply labels the object in a different way
    - It either expands or narrows the methods / properties we can access from an object, because we're going from more specific to less specific
- Explicit casting, going froom parent to child type, is also known as downcasting, since we're going down the hierarchy from parent to child
