...
- `toString()` : the toString() method inherited from the Object class is automatically called when we print an reference variable.  If this method is not overrident (polymorphism), it will print the output as `<fully qualified class name>@hashedMemoryAddress`
- `equals(Object o)` : compares two objects. If .equals(Object o) is not being overriden, then it behavaes exactly the same as `==` (aka, it will compare memory addresses). However, we often override .equals(Object o) in order to provide the ability to compare values of objects instead.
...
- **If we override equals(), hashCode() should also be overriden accordingly to the properties
