...
- Accomplished using
    ...
    - We cannot change a String object's actual value, so a new String must be generated with the desired <u>modifications</u>.
    ...
    - `charAt(int index)` : return a char at a given position i the string
    - `matches(String regex)` : returns true if the string matches a given **regular expression**
    - `substring(int beginIndex)` : returns a portion of the stirng as another string starting at some index (indices start at 0)
